...
routes.get('/', (req, res) =>{
    req.getConnection((err, conn)=>{
        ...
        conn.query('SELECT ACTIVIDADES.idActividades, ACTIVIDADES.nombre, ACTIVIDADES.categoria, ACTIVIDADES.tag, ACTIVIDADES.detalles, ACTIVIDADES.level, CURSOS.titulo as Curso, CURSOS.precio FROM ACTIVIDADES inner join CURSOS on ACTIVIDADES.fk_curso = CURSOS.idCurso', (err, rows)=>{
                    if(err) return res.send(err)
                    res.json(rows)
                })
    })
